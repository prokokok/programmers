...
def n_queen(n, sol):
    ...
    while stack:
        ...
        # 같은 열 지우기
        candidate_set.remove(col)

        # 대각선값 지우기
        diag_colunm_1 = next_row - row + col
        diag_colunm_2 = row + col - next_row

        if diag_colunm_1 in candidate_set:
            candidate_set.remove(diag_colunm_1)
        if diag_colunm_2 in candidate_set:
            candidate_set.remove(diag_colunm_2)

        if candidate_set:
            for candidate in candidate_set:
                stack.append((next_row, candidate))
        print(row, candidate_set)
    #     return 0
    # return cnt
